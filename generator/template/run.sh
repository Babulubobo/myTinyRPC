...
PROJECT_CONF_FILE="../conf/myRPC.xml"


echo "Run myRPC project, name: ${PROJECT_NAME}, path: ${PROJECT_BIN_FILE}"
...
then
  echo "Run myRPC server eror, file: ${PROJECT_BIN_FILE} not exist, please check file"
  exit -1
fi
...
then
  echo "Run myRPC error, file: ${PROJECT_CONF_FILE} not exist, please check config file"
  exit -1
fi
...
echo "Start myRPC server ${PROJECT_CONF_FILE} succ"
