...
void TinyPBCoder::decode(std::vector<AbstractProtocol::s_ptr>& out_messages, TcpBuffer::s_ptr buffer) {
    ...
    while(1) {
        ...
        if(parse_success) {
            ...
            int method_name_len_index = req_id_index + message->m_req_id_len; // no sizeof 
...
const char* TinyPBCoder::encodeTinyPB(std::shared_ptr<TinyPBProtocal> message, int& len) {
    ...
    int pk_len = 2 + 24 +  message->m_method_name.length() + message->m_req_id.length() + message->m_err_info.length() + message->m_pb_data.length();
